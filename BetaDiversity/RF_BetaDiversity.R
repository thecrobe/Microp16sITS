...
# LOAD & PROCESS DATA
# =============================================================================
load("/Users/justinstewart/Dropbox/Collaborations/FamilyExperiment/Data/VU16s_roottraits_updated.Rdata")
...
load("/Users/justinstewart/Dropbox/Collaborations/FamilyExperiment/Data/VU_ITS_roottraits_updated.Rdata")
...
# =============================================================================
# Run models with LOOCV
# =============================================================================
train_models <- function(metadata, root_traits) {
  ...
  models <- list(
    Roots_Only_PCoA1       = train(formula_roots,       data = metadata, method = "rf",
                                   trControl = train_ctrl, importance = TRUE),
    Roots_Only_PCoA2       = train(formula_roots_pcoa2, data = metadata, method = "rf",
                                   trControl = train_ctrl, importance = TRUE),
    
    PlantShort_Only_PCoA1  = train(formula_plant,       data = metadata, method = "rf",
                                   trControl = train_ctrl, importance = TRUE),
    PlantShort_Only_PCoA2  = train(formula_plant_pcoa2, data = metadata, method = "rf",
                                   trControl = train_ctrl, importance = TRUE),
    
    All_Predictors_PCoA1   = train(formula_all,         data = metadata, method = "rf",
                                   trControl = train_ctrl, importance = TRUE),
    All_Predictors_PCoA2   = train(formula_all_pcoa2,   data = metadata, method = "rf",
                                   trControl = train_ctrl, importance = TRUE)
  )
...
write.csv(performance_results, file = file.path(save_dir, "RF_Model_Performance_LOO.csv"), row.names = FALSE)
