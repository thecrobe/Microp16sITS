...
# =============================================================================
# Load and process Fungi data
# =============================================================================
load("/VU_ITS_roottraits_updated.Rdata")
metadata_fungi_pcoa <- data.frame(sample_data(VU_ITS_filtered_joined))
metadata_fungi_pcoa$N__root_percent <- as.numeric(metadata_fungi_pcoa$N__root_percent)

metadata_fungi_pcoa <- metadata_fungi_pcoa %>%
  mutate(
...
physeq_obj <- prune_samples(rownames(metadata_fungi_pcoa), VU_ITS_filtered_joined)
...
metadata_fungi_pcoa <- bind_cols(metadata_fungi_pcoa, pcoa_scores)
...
data_fungi <- metadata_fungi_pcoa[, c("PCoA1", "PCoA2", predictors, "PlantShort")]
data_fungi$PlantShort <- factor(data_fungi$PlantShort, levels = levels(metadata_fungi_pcoa$PlantShort))

data_fungi_filtered <- scale_center_filter(data_fungi, predictors) %>%
  filter(abs(C_root_percent) <= 2)

X_fungi_filtered <- data_fungi_filtered[, feature_names]
...
ggplot(err_df, aes(x = Trees, y = OOB_Error)) +
  ...
  theme_minimal() +
  labs(
    title = "Random Forest convergence - Fungi",
    x = "Number of trees",
...
# =============================================================================
predictor_values <- data_fungi_filtered %>%
...
for (feat in features) {
  p <- ggplot(shap_with_values %>% filter(Feature == feat), 
    ...
    theme_minimal() +
    labs(
      title = paste("SHAP Values vs Scaled", feat, "(Fungi)"),
  ...
  ggsave(
    filename = file.path(shap_save_dir, paste0("SHAP_Scaled_", feat, "_Fungi.pdf")),
    plot = p,device = "pdf",
    width = 6,
    height = 4,
    dpi = 300
...
multi_panel_plot <- ggplot(shap_with_values, aes(x = .data[[Feature]], y = SHAP, color = PC)) +
  ...
  theme_minimal() +
  labs(
    title = "SHAP Values vs Scaled Trait Values (Fungi)",
...
ggsave(
  filename = file.path(shap_save_dir, "SHAP_All_Traits_Scaled_Fungi.pdf"),
...
# Save plot
ggsave(
  filename = file.path(shap_save_dir, "SHAP_Mean_Absolute_Barplot_Fungi.pdf"),
  width = 8,
  height = 6,device = "pdf",
  dpi = 300,scale=0.5
)
